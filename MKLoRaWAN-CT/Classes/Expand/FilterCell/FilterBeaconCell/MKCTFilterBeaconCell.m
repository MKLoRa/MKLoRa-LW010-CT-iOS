...
@implementation MKCTFilterBeaconCellModel
@end

@interface MKCTFilterBeaconCell ()
...
@implementation MKCTFilterBeaconCell

+ (MKCTFilterBeaconCell *)initCellWithTableView:(UITableView *)tableView {
    MKCTFilterBeaconCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCTFilterBeaconCellIdenty"];
    if (!cell) {
        cell = [[MKCTFilterBeaconCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCTFilterBeaconCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKCTFilterBeaconCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCTFilterBeaconCellModel.class]) {
        return;
    }
...
- (MKTextField *)minTextField {
    if (!_minTextField) {
        ...
        _minTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_ct_beaconMinValueChanged:index:)]) {
                [self.delegate mk_ct_beaconMinValueChanged:text index:self.dataModel.index];
            }
        };
    }
...
- (MKTextField *)maxTextField {
    if (!_maxTextField) {
        ...
        _maxTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_ct_beaconMaxValueChanged:index:)]) {
                [self.delegate mk_ct_beaconMaxValueChanged:text index:self.dataModel.index];
            }
        };
    }
