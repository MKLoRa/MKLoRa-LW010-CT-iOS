...
@implementation MKCTTextButtonCellModel
@end

@interface MKCTTextButtonCell ()
...
@implementation MKCTTextButtonCell

+ (MKCTTextButtonCell *)initCellWithTableView:(UITableView *)tableView {
    MKCTTextButtonCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCTTextButtonCellIdenty"];
    if (!cell) {
        cell = [[MKCTTextButtonCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCTTextButtonCellIdenty"];
    }
    return cell;
}
...
- (void)rightButtonPressed {
    if ([self.delegate respondsToSelector:@selector(ct_textButtonCell_buttonAction:)]) {
        [self.delegate ct_textButtonCell_buttonAction:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCTTextButtonCellModel *)dataModel {
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCTTextButtonCellModel.class]) {
        return;
    }
