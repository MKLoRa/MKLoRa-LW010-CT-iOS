...
/**
 线程ID
 */
@property (nonatomic, assign)mk_ct_taskOperationID operationID;
...
- (instancetype)initOperationWithID:(mk_ct_taskOperationID)operationID
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic {
    [self dataParserReceivedData:[MKCTTaskAdopter parseReadDataWithCharacteristic:characteristic]];
}

- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic {
    [self dataParserReceivedData:[MKCTTaskAdopter parseWriteDataWithCharacteristic:characteristic]];
}
...
- (void)dataParserReceivedData:(NSDictionary *)dataDic{
    ...
    mk_ct_taskOperationID operationID = [dataDic[@"operationID"] integerValue];
    if (operationID == mk_ct_defaultTaskOperationID || operationID != self.operationID) {
        return;
    }
    ...
    //分帧数据
    NSString *totalNum = returnData[mk_ct_totalNumKey];
    if (MKValidStr(totalNum)) {
        //存在分帧数据
        self.receiveTimerCount = 0;
        NSData *data = returnData[mk_ct_contentKey];
        if (data) {
