//
//  MKCTIndicatorSettingsController.m
...
@interface MKCTIndicatorSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCTIndicatorSettingsModel *dataModel;

@end

@implementation MKCTIndicatorSettingsController

- (void)dealloc {
    NSLog(@"MKCTIndicatorSettingsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-CT", @"MKCTIndicatorSettingsController", @"ct_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCTIndicatorSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCTIndicatorSettingsModel alloc] init];
    }
    return _dataModel;
}
