...
MKCTFilterRelationshipCellDelegate>
...
@property (nonatomic, strong)MKCTBleFixDataModel *dataModel;

@end

@implementation MKCTBleFixController

- (void)dealloc {
    NSLog(@"MKCTBleFixController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 4 && indexPath.row == 2) {
        //Filter by Raw Data
        MKCTFilterByRawDataController *vc = [[MKCTFilterByRawDataController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKCTFilterRelationshipCell *cell = [MKCTFilterRelationshipCell initCellWithTableView:tableView];
...
#pragma mark - MKCTFilterRelationshipCellDelegate
- (void)ct_filterRelationshipChanged:(NSInteger)index dataListIndex:(NSInteger)dataListIndex {
    if (index == 0) {
        ...
        MKCTFilterRelationshipCellModel *cellModel = self.section1List[0];
    ...
    if (index == 1) {
        ...
        MKCTFilterRelationshipCellModel *cellModel = self.section3List[0];
        cellModel.dataListIndex = dataListIndex;
        return;
    }
}
...
- (void)loadSection1Datas {
    MKCTFilterRelationshipCellModel *cellModel1 = [[MKCTFilterRelationshipCellModel alloc] init];
    cellModel1.index = 0;
...
- (void)loadSection3Datas {
    MKCTFilterRelationshipCellModel *cellModel = [[MKCTFilterRelationshipCellModel alloc] init];
    cellModel.index = 1;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-CT", @"MKCTBleFixController", @"ct_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCTBleFixDataModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCTBleFixDataModel alloc] init];
    }
    return _dataModel;
}
