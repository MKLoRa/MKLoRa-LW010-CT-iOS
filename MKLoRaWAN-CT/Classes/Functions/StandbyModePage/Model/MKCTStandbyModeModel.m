...
@interface MKCTStandbyModeModel ()
...
@implementation MKCTStandbyModeModel
...
- (BOOL)readStrategy {
    ...
    [MKCTInterface ct_readStandbyModePositioningStrategyWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configStrategy {
    __block BOOL success = NO;
    [MKCTInterface ct_configStandbyModePositioningStrategy:self.strategy sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
