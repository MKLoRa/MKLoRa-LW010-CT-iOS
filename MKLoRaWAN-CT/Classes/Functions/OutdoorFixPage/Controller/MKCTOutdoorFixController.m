...
@interface MKCTOutdoorFixController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCTOutdoorFixModel *dataModel;

@end

@implementation MKCTOutdoorFixController

- (void)dealloc {
    NSLog(@"MKCTOutdoorFixController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-CT", @"MKCTOutdoorFixController", @"ct_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCTOutdoorFixModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCTOutdoorFixModel alloc] init];
    }
    return _dataModel;
}
