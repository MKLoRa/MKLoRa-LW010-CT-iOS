...
@implementation MKCTOutdoorFixModel
...
- (BOOL)readBleInterval {
    ...
    [MKCTInterface ct_readOutdoorBLEReportIntervalWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configBleInterval {
    __block BOOL success = NO;
    [MKCTInterface ct_configOutdoorBLEReportInterval:[self.bleInterval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readGpsInterval {
    __block BOOL success = NO;
    [MKCTInterface ct_readOutdoorGPSReportIntervalWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configGpsInterval {
    __block BOOL success = NO;
    [MKCTInterface ct_configOutdoorGPSReportInterval:[self.gpsInterval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
