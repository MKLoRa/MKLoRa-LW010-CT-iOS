...
@interface MKCTDeviceInfoController ()<UITableViewDelegate,
UITableViewDataSource,
MKCTTextButtonCellDelegate>
...
@property (nonatomic, strong)MKCTDeviceInfoModel *dataModel;
...
@implementation MKCTDeviceInfoController

- (void)dealloc {
    NSLog(@"MKCTDeviceInfoController销毁");
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(deviceStartDFUProcess)
                                                 name:@"mk_ct_startDfuProcessNotification"
                                               object:nil];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 5 && indexPath.row == 0) {
        //Debugger Mode
        MKCTDebuggerController *vc = [[MKCTDebuggerController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKCTTextButtonCell *cell = [MKCTTextButtonCell initCellWithTableView:tableView];
...
#pragma mark - MKCTTextButtonCellDelegate
/// 用户点击了右侧按钮
/// @param index cell所在序列号
- (void)ct_textButtonCell_buttonAction:(NSInteger)index {
    if (index == 0) {
        //DFU
        MKCTUpdateController *vc = [[MKCTUpdateController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)pushSelftestInterface {
    MKCTSelftestController *vc = [[MKCTSelftestController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)updateCellDatas {
    ...
    if (ValidStr(self.dataModel.firmware)) {
        MKCTTextButtonCellModel *firmwareModel = self.section1List[0];
...
- (void)loadSection1Datas {
    MKCTTextButtonCellModel *cellModel = [[MKCTTextButtonCellModel alloc] init];
    cellModel.index = 0;
...
- (MKCTDeviceInfoModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCTDeviceInfoModel alloc] init];
    }
    return _dataModel;
}
