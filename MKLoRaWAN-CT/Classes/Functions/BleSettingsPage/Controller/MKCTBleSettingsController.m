...
@interface MKCTBleSettingsController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
mk_textSwitchCellDelegate,
MKCTBroadcastTxPowerCellDelegate>
...
@property (nonatomic, strong)MKCTBleSettingsModel *dataModel;
...
@implementation MKCTBleSettingsController

- (void)dealloc {
    NSLog(@"MKCTBleSettingsController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 5 && indexPath.row == 0 && [MKCTConnectModel shared].hasPassword) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKCTBroadcastTxPowerCell *cell = [MKCTBroadcastTxPowerCell initCellWithTableView:tableView];
...
#pragma mark - MKCTBroadcastTxPowerCellDelegate
/*
...
- (void)ct_txPowerValueChanged:(NSInteger)txPower {
    ...
    MKCTBroadcastTxPowerCellModel *cellModel = self.section1List[0];
...
- (void)configPassword{
    ...
    [alertView showAlertWithTitle:@"Change Password" message:msg notificationName:@"mk_ct_needDismissAlert"];
}
...
- (void)setPasswordToDevice{
    ...
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                                     inView:self.view
                              isPenetration:NO];
    [MKCTInterface ct_configPassword:password sucBlock:^{
        [[MKHudManager share] hide];
        [self.view showCentralToast:@"Success"];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
...
- (void)loadSection1Datas {
    MKCTBroadcastTxPowerCellModel *cellModel = [[MKCTBroadcastTxPowerCellModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-CT", @"MKCTBleSettingsController", @"ct_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCTBleSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCTBleSettingsModel alloc] init];
    }
    return _dataModel;
}
