...
@interface MKCTFilterByPirController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCTFilterByPirModel *dataModel;

@end

@implementation MKCTFilterByPirController

- (void)dealloc {
    NSLog(@"MKCTFilterByPirController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-CT", @"MKCTFilterByPirController", @"ct_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCTFilterByPirModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCTFilterByPirModel alloc] init];
    }
    return _dataModel;
}
