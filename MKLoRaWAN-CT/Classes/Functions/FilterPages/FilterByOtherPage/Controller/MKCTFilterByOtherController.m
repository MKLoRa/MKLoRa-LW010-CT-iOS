...
@interface MKCTFilterByOtherController ()<UITableViewDelegate,
UITableViewDataSource,
mk_textSwitchCellDelegate,
MKCTFilterEditSectionHeaderViewDelegate,
MKCTFilterByRawDataCellDelegate,
...
@property (nonatomic, strong)MKCTFilterByOtherModel *dataModel;

@end

@implementation MKCTFilterByOtherController

- (void)dealloc {
    NSLog(@"MKCTFilterByOtherController销毁");
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1) {
        ...
        MKCTFilterEditSectionHeaderViewModel *model = [[MKCTFilterEditSectionHeaderViewModel alloc] init];
        model.index = 0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKCTFilterByRawDataCell *cell = [MKCTFilterByRawDataCell initCellWithTableView:tableView];
...
- (void)mk_ct_filterEditSectionHeaderView_addButtonPressed:(NSInteger)index {
    ...
    MKCTFilterByRawDataCellModel *cellModel = [[MKCTFilterByRawDataCellModel alloc] init];
    if (cellModelIndex == 0) {
        //Condition A
...
#pragma mark - MKCTFilterByRawDataCellDelegate
/// 输入框内容发生改变
/// @param textType 哪个输入框发生改变了
/// @param index 当前cell所在的row
/// @param textValue 当前textField内容
- (void)ct_rawFilterDataChanged:(mk_ct_filterRawAdvDataTextType)textType
                          index:(NSInteger)index
                      textValue:(NSString *)textValue {
    if (index >= self.section1List.count) {
        return;
    }
    MKCTFilterByRawDataCellModel *cellModel = self.section1List[index];
    if (textType == mk_ct_filterRawAdvDataTextTypeDataType) {
        //过滤类型输入框内容发生改变
        cellModel.dataType = textValue;
        return;
    }
    if (textType == mk_ct_filterRawAdvDataTextTypeMinIndex) {
        //开始过滤的Byte索引输入框发生改变
        cellModel.minIndex = textValue;
        return;
    }
    if (textType == mk_ct_filterRawAdvDataTextTypeMaxIndex) {
        //截止过滤的Byte索引输入框发生改变
        cellModel.maxIndex = textValue;
        return;
    }
    if (textType == mk_ct_filterRawAdvDataTextTypeRawDataType) {
        //过滤内容输入框发生改变
...
- (void)configDataToDevice {
    ...
    for (NSInteger i = 0; i < self.section1List.count; i ++) {
        MKCTFilterByRawDataCellModel *cellModel = self.section1List[i];
        MKCTFilterRawAdvDataModel *model = [[MKCTFilterRawAdvDataModel alloc] init];
...
- (mk_ct_filterByOther)loadCurrentRelationship {
    MKTextButtonCellModel *cellModel = self.section2List[0];
    if (self.section1List.count == 1) {
        //@[@"A"];
        return mk_ct_filterByOther_A;
    }
    if (self.section1List.count == 2) {
        //@[@"A & B",@"A | B"]
        if (cellModel.dataListIndex == 0) {
            return mk_ct_filterByOther_AB;
        }
        return mk_ct_filterByOther_AOrB;
    }
    if (self.section1List.count == 3) {
        //@[@"A & B & C",@"(A & B) | C",@"A | B | C"]
        if (cellModel.dataListIndex == 0) {
            return mk_ct_filterByOther_ABC;
        }
        if (cellModel.dataListIndex == 1) {
            return mk_ct_filterByOther_ABOrC;
        }
        if (cellModel.dataListIndex == 2) {
            return mk_ct_filterByOther_AOrBOrC;
        }
    }
    return mk_ct_filterByOther_AOrBOrC;
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-CT", @"MKCTFilterByOtherController", @"ct_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCTFilterByOtherModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCTFilterByOtherModel alloc] init];
    }
    return _dataModel;
}
