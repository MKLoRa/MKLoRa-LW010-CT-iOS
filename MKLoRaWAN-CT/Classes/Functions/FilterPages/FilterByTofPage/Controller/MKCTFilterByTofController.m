...
@interface MKCTFilterByTofController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCTFilterByTofModel *dataModel;

@end

@implementation MKCTFilterByTofController

- (void)dealloc {
    NSLog(@"MKCTFilterByTofController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-CT", @"MKCTFilterByTofController", @"ct_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCTFilterByTofModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCTFilterByTofModel alloc] init];
    }
    return _dataModel;
}
