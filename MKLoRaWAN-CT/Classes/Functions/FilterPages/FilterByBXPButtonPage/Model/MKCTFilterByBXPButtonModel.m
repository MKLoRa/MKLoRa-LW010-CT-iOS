...
@interface MKCTFilterByBXPButtonModel ()
...
@implementation MKCTFilterByBXPButtonModel
...
- (BOOL)readFilterStatus {
    ...
    [MKCTInterface ct_readBXPButtonFilterStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterStatus {
    __block BOOL success = NO;
    [MKCTInterface ct_configFilterByBXPButtonStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readButtonAlarmContent {
    __block BOOL success = NO;
    [MKCTInterface ct_readBXPButtonAlarmFilterStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configBXPButtonContent {
    __block BOOL success = NO;
    [MKCTInterface ct_configFilterByBXPButtonAlarmStatus:self.singlePress
