...
@interface MKCTFilterByMacController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCTFilterByMacModel *dataModel;

@end

@implementation MKCTFilterByMacController

- (void)dealloc {
    NSLog(@"MKCTFilterByMacController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-CT", @"MKCTFilterByMacController", @"ct_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCTFilterByMacModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCTFilterByMacModel alloc] init];
    }
    return _dataModel;
}
