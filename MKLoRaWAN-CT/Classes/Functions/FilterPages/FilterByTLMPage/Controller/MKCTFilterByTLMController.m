...
@interface MKCTFilterByTLMController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCTFilterByTLMModel *dataModel;

@end

@implementation MKCTFilterByTLMController

- (void)dealloc {
    NSLog(@"MKCTFilterByTLMController销毁");
}
...
- (void)configFilterStatus:(BOOL)isOn {
    ...
    [MKCTInterface ct_configFilterByTLMStatus:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (void)configTLMVersion:(NSInteger)version {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCTInterface ct_configFilterByTLMVersion:version sucBlock:^{
        [[MKHudManager share] hide];
...
- (MKCTFilterByTLMModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCTFilterByTLMModel alloc] init];
    }
    return _dataModel;
}
