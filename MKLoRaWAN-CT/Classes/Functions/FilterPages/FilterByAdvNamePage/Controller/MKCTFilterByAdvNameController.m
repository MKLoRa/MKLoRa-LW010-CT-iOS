//
//  MKCTFilterByAdvNameController.m
...
@interface MKCTFilterByAdvNameController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCTFilterByAdvNameModel *dataModel;

@end

@implementation MKCTFilterByAdvNameController

- (void)dealloc {
    NSLog(@"MKCTFilterByAdvNameController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-CT", @"MKCTFilterByAdvNameController", @"ct_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCTFilterByAdvNameModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCTFilterByAdvNameModel alloc] init];
    }
    return _dataModel;
}
