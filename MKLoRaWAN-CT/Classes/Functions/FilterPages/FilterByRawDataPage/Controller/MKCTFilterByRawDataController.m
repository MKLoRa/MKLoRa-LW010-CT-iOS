//
//  MKCTFilterByRawDataController.m
...
@property (nonatomic, strong)MKCTFilterByRawDataModel *dataModel;

@end

@implementation MKCTFilterByRawDataController

- (void)dealloc {
    NSLog(@"MKCTFilterByRawDataController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //iBeacon
        MKCTFilterByBeaconController *vc = [[MKCTFilterByBeaconController alloc] init];
        vc.pageType = mk_ct_filterByBeaconPageType_beacon;
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //Eddystone-UID
        MKCTFilterByUIDController *vc = [[MKCTFilterByUIDController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 2) {
        //Eddystone-URL
        MKCTFilterByURLController *vc = [[MKCTFilterByURLController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 3) {
        //Eddystone-TLM
        MKCTFilterByTLMController *vc = [[MKCTFilterByTLMController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 4) {
        //BXP-iBeacon
        MKCTFilterByBeaconController *vc = [[MKCTFilterByBeaconController alloc] init];
        vc.pageType = mk_ct_filterByBeaconPageType_bxpBeacon;
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 0) {
        //BXP-Button
        MKCTFilterByBXPButtonController *vc = [[MKCTFilterByBXPButtonController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 1) {
        //BXP - Tag
        MKCTFilterByBXPTagController *vc = [[MKCTFilterByBXPTagController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 2) {
        //MK-PIR
        MKCTFilterByPirController *vc = [[MKCTFilterByPirController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 3) {
        //MK-TOF
        MKCTFilterByTofController *vc = [[MKCTFilterByTofController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
...
- (void)configDeviceInfo:(BOOL)isOn {
    ...
    [MKCTInterface ct_configFilterByBXPDeviceInfoStatus:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (void)configBXPAcc:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCTInterface ct_configBXPAccFilterStatus:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (void)configBXPTH:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCTInterface ct_configBXPTHFilterStatus:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (MKCTFilterByRawDataModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCTFilterByRawDataModel alloc] init];
    }
    return _dataModel;
}
