...
@implementation MKCTDeviceModeController

- (void)dealloc {
    NSLog(@"MKCTDeviceModeController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1 && indexPath.row == 0) {
        //Standby Mode
        MKCTStandbyModeModelController *vc = [[MKCTStandbyModeModelController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 1) {
        //Timing Mode
        MKCTTimingModeController *vc = [[MKCTTimingModeController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 2) {
        //Periodic Mode
        MKCTPeriodicModeController *vc = [[MKCTPeriodicModeController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
...
- (void)readDataFromDevice {
    ...
    [MKCTInterface ct_readWorkModeWithSucBlock:^(id  _Nonnull returnData) {
...
- (void)saveWorkModeToDevice:(NSInteger)mode {
    ...
    [MKCTInterface ct_configWorkMode:mode sucBlock:^{
        [[MKHudManager share] hide];
