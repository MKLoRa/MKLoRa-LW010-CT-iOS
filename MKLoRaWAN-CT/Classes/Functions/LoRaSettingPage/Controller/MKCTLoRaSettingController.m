...
@interface MKCTLoRaSettingController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCTLoRaSettingModel *dataModel;

@end

@implementation MKCTLoRaSettingController

- (void)dealloc {
    NSLog(@"MKCTLoRaSettingController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-CT", @"MKCTLoRaSettingController", @"ct_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCTLoRaSettingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCTLoRaSettingModel alloc] init];
    }
    return _dataModel;
}
