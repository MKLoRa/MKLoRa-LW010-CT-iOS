...
@implementation MKCTTimingModeTimePointModel
@end

@interface MKCTTimingModeModel ()
...
@implementation MKCTTimingModeModel
...
- (BOOL)readStrategy {
    ...
    [MKCTInterface ct_readTimingModePositioningStrategyWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configStrategy {
    __block BOOL success = NO;
    [MKCTInterface ct_configTimingModePositioningStrategy:self.strategy sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readReportingTimePoint {
    __block BOOL success = NO;
    [MKCTInterface ct_readTimingModeReportingTimePointWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configReportingTimePoint:(NSArray <MKCTTimingModeTimePointModel *>*)pointList {
    __block BOOL success = NO;
    [MKCTInterface ct_configTimingModeReportingTimePoint:pointList sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
