...
@interface MKCTDebuggerController ()<UITableViewDelegate,
UITableViewDataSource,
MFMailComposeViewControllerDelegate,
mk_ct_centralManagerLogDelegate,
MKCTDebuggerCellDelegate>
...
@property (nonatomic, strong)MKCTDebuggerButton *deleteButton;

@property (nonatomic, strong)MKCTDebuggerButton *exportButton;
...
@implementation MKCTDebuggerController

- (void)dealloc {
    NSLog(@"MKCTDebuggerController销毁");
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_ct_stopDebuggerMode" object:nil];
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self loadSubViews];
    [MKCTCentralManager shared].logDelegate = self;
    [self readDataFromLocal];
    //进入debugger模式之后，即使设备断开连接，也要停留在当前页面
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_ct_startDebuggerMode" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(deviceDisconnect)
                                                 name:mk_ct_peripheralConnectStateChangedNotification
                                               object:nil];
}

#pragma mark - super method
- (void)leftButtonMethod {
    if ([MKCTCentralManager shared].connectStatus != mk_ct_centralConnectStatusConnected) {
        //当前设备已经断开连接，则返回扫描页面
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_ct_popToRootViewControllerNotification" object:nil];
        return;
    }
    //当前设备正常连接，返回上一级页面
    self.leftAction = YES;
    [[MKCTCentralManager shared] notifyLogData:NO];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKCTDebuggerCell *cell = [MKCTDebuggerCell initCellWithTableView:tableView];
...
#pragma mark - mk_ct_centralManagerLogDelegate
...
#pragma mark - MKCTDebuggerCellDelegate
- (void)ct_debuggerCellSelectedChanged:(NSInteger)index selected:(BOOL)selected {
    ...
    MKCTDebuggerCellModel *cellModel = self.dataList[index];
...
- (void)deviceDisconnect {
    ...
    [self.syncIcon.layer removeAnimationForKey:@"mk_ct_syncAnimation"];
    self.isLoading = NO;
...
- (void)startButtonPressed {
    ...
    self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-CT", @"MKCTDebuggerController", @"ct_delete_disableIcon.png");
    self.exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-CT", @"MKCTDebuggerController", @"ct_export_disableIcon.png");
    [self.startButton setTitle:@"Stop" forState:UIControlStateNormal];
    [self.contentList removeAllObjects];
    [[MKCTCentralManager shared] notifyLogData:YES];
    [self.syncIcon.layer addAnimation:[MKCustomUIAdopter refreshAnimation:1.f]
                               forKey:@"mk_ct_syncAnimation"];
}

- (void)deleteButtonPressed {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKCTLogDatabaseManager deleteDatasWithMacAddress:self.macAddress sucBlock:^{
        NSMutableArray *tempList = [NSMutableArray array];
        for (NSInteger i = 0; i < self.dataList.count; i ++) {
            MKCTDebuggerCellModel *cellModel = self.dataList[i];
...
- (void)readDataFromLocal {
    ...
    [MKCTLogDatabaseManager readLocalLogsWithMacAddress:self.macAddress sucBlock:^(NSArray<NSDictionary *> * _Nonnull logList) {
        [[MKHudManager share] hide];
...
- (void)loadTableViewDatas:(NSArray *)logList {
    ...
    for (NSInteger i = 0; i < logList.count; i ++) {
        ...
        MKCTDebuggerCellModel *cellModel = [[MKCTDebuggerCellModel alloc] init];
...
- (void)saveDataListToLocal {
    ...
    [MKCTLogDatabaseManager insertLogDatas:tempList macAddress:self.macAddress sucBlock:^{
        [[MKHudManager share] hide];
        if (self.leftAction) {
...
- (void)updateTopButtonState {
    BOOL enable = NO;
    for (MKCTDebuggerCellModel *cellModel in self.dataList) {
        if (cellModel.selected) {
            enable = YES;
            break;
        }
    ...
    if (enable) {
        //选中了，可以删除或者导出
        self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-CT", @"MKCTDebuggerController", @"ct_delete_enableIcon.png");
        self.exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-CT", @"MKCTDebuggerController", @"ct_export_enableIcon.png");
        return;
    }
    //不存在选中的
    self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-CT", @"MKCTDebuggerController", @"ct_delete_disableIcon.png");
...
- (void)showTipsAlert:(NSString *)msg {
    ...
    [alertView showAlertWithTitle:@"Tips!" message:msg notificationName:@"mk_ct_needDismissAlert"];
}
...
- (UIImageView *)syncIcon {
    if (!_syncIcon) {
        ...
        _syncIcon.image = LOADICON(@"MKLoRaWAN-CT", @"MKCTDebuggerController", @"ct_sync_disableIcon.png");
    }
    return _syncIcon;
}

- (MKCTDebuggerButton *)deleteButton {
    if (!_deleteButton) {
        _deleteButton = [[MKCTDebuggerButton alloc] init];
        _deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-CT", @"MKCTDebuggerController", @"ct_delete_disableIcon.png");
...
- (MKCTDebuggerButton *)exportButton {
    if (!_exportButton) {
        _exportButton = [[MKCTDebuggerButton alloc] init];
        _exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-CT", @"MKCTDebuggerController", @"ct_export_disableIcon.png");
