...
@interface MKCTFilterByTLMModel ()
...
@implementation MKCTFilterByTLMModel
...
- (void)configFilterStatus:(BOOL)isOn
    ...
    [MKCTInterface ct_configFilterByTLMStatus:isOn
                                     sucBlock:sucBlock
...
- (void)configTLMVersion:(NSInteger)version
                sucBlock:(void (^)(void))sucBlock
             failedBlock:(void (^)(NSError *error))failedBlock {
    [MKCTInterface ct_configFilterByTLMVersion:version
                                      sucBlock:sucBlock
                                   failedBlock:failedBlock];
}
...
- (BOOL)readFilterStatus {
    ...
    [MKCTInterface ct_readFilterByTLMStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterStatus {
    __block BOOL success = NO;
    [MKCTInterface ct_configFilterByTLMStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readTLMVersion {
    __block BOOL success = NO;
    [MKCTInterface ct_readFilterByTLMVersionWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configTLMVersion {
    __block BOOL success = NO;
    [MKCTInterface ct_configFilterByTLMVersion:self.tlm sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
