//
//  MKCTTempMonitorSettingsController.m
...
@interface MKCTTempMonitorSettingsController ()<UITableViewDelegate,
UITableViewDataSource,
mk_textSwitchCellDelegate,
MKTextFieldCellDelegate,
MKCTTempMonitorThresholdCellDelegate>
...
@property (nonatomic, strong)MKCTTempMonitorSettingsModel *dataModel;

@end

@implementation MKCTTempMonitorSettingsController

- (void)dealloc {
    NSLog(@"MKCTTempMonitorSettingsController销毁");
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 2) {
        MKCTTempMonitorValueCell *cell = [MKCTTempMonitorValueCell initCellWithTableView:tableView];
    ...
    MKCTTempMonitorThresholdCell *cell = [MKCTTempMonitorThresholdCell initCellWithTableView:tableView];
...
- (void)mk_textSwitchCellStatusChanged:(BOOL)isOn index:(NSInteger)index {
    ...
    if (index == 1) {
        //Temp Threshold Alarm Swicth
        self.dataModel.alarmSwitch = isOn;
...
#pragma mark - MKCTTempMonitorThresholdCellDelegate
- (void)ct_tempMonitorThresholdCell_valueChanged:(NSInteger)index value:(NSString *)value {
    if (index == 0) {
        //Max.
        self.dataModel.maxThreshold = value;
        MKCTTempMonitorThresholdCellModel *cellModel = self.section4List[0];
        cellModel.value = value;
        return;
    }
    if (index == 1) {
        //Min.
        self.dataModel.minThreshold = value;
        MKCTTempMonitorThresholdCellModel *cellModel = self.section4List[1];
        cellModel.value = value;
        return;
    }
}
...
- (void)loadSection3Datas {
    MKTextSwitchCellModel *cellModel = [[MKTextSwitchCellModel alloc] init];
    cellModel.index = 1;
    cellModel.msg = @"Temp Threshold Alarm Swicth";
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"Temperature Monitor Settings";
    self.titleLabel.font = MKFont(15.f);
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-CT", @"MKCTTempMonitorSettingsController", @"ct_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCTTempMonitorSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCTTempMonitorSettingsModel alloc] init];
    }
    return _dataModel;
}
