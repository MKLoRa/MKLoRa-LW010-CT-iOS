...
@implementation MKCTTimeSegmentedCellModel
@end

@interface MKCTTimeSegmentedCell ()
...
@implementation MKCTTimeSegmentedCell

+ (MKCTTimeSegmentedCell *)initCellWithTableView:(UITableView *)tableView {
    MKCTTimeSegmentedCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCTTimeSegmentedCellIdenty"];
    if (!cell) {
        cell = [[MKCTTimeSegmentedCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCTTimeSegmentedCellIdenty"];
    }
    return cell;
}
...
- (void)deleteButtonPressed {
    if ([self.delegate respondsToSelector:@selector(ct_timeSegmentedCell_deleteButtonPressed:)]) {
        [self.delegate ct_timeSegmentedCell_deleteButtonPressed:self.dataModel.index];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKCTTimeSegmentedCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCTTimeSegmentedCellModel.class]) {
        return;
    }
...
- (void)miniuteButtonPressed:(BOOL)start {
    ...
    [pickView showPickViewWithDataList:self.minuteGearList selectedRow:selectedRow block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(ct_timeSegmentedCell_minuteGearButtonPressed:start:minuteGear:)]) {
            [self.delegate ct_timeSegmentedCell_minuteGearButtonPressed:self.dataModel.index start:start minuteGear:currentRow];
        }
    }];
}
...
- (MKTextField *)intervalTextField {
    if (!_intervalTextField) {
        ...
        _intervalTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(ct_timeSegmentedCell_reportIntervalChanged:interval:)]) {
                [self.delegate ct_timeSegmentedCell_reportIntervalChanged:self.dataModel.index interval:text];
            }
        };
    }
