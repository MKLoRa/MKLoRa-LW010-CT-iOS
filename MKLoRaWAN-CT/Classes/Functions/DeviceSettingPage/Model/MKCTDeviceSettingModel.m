...
@interface MKCTDeviceSettingModel ()
...
@implementation MKCTDeviceSettingModel

- (void)readDataWithSucBlock:(void (^)(void))sucBlock failedBlock:(void (^)(NSError *error))failedBlock {
    dispatch_async(self.readQueue, ^{
        if (![self readBuzzerType]) {
            [self operationFailedBlockWithMsg:@"Read Buzzer Type Error" block:failedBlock];
            return;
        }
...
- (void)configDataWithSucBlock:(void (^)(void))sucBlock failedBlock:(void (^)(NSError *error))failedBlock {
    dispatch_async(self.readQueue, ^{
        ...
        if (![self configBuzzerType]) {
            [self operationFailedBlockWithMsg:@"Config Buzzer Type Error" block:failedBlock];
            return;
        }
...
- (BOOL)readBuzzerType {
    ...
    [MKCTInterface ct_readBuzzerSoundTypeWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configBuzzerType {
    __block BOOL success = NO;
    [MKCTInterface ct_configBuzzerSoundType:self.buzzer sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readTimeZone {
    __block BOOL success = NO;
    [MKCTInterface ct_readTimeZoneWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configTimeZone {
    __block BOOL success = NO;
    [MKCTInterface ct_configTimeZone:(self.timeZone - 24) sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readLowPowerPayload {
    __block BOOL success = NO;
    [MKCTInterface ct_readLowPowerPayloadStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configLowPowerPayload {
    __block BOOL success = NO;
    [MKCTInterface ct_configLowPowerPayloadStatus:self.lowPowerPayload sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readLowPowerPrompt {
    __block BOOL success = NO;
    [MKCTInterface ct_readLowPowerPromptWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configLowPowerPrompt {
    __block BOOL success = NO;
    [MKCTInterface ct_configLowPowerPrompt:self.prompt sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readLowPowerPayloadInterval {
    __block BOOL success = NO;
    [MKCTInterface ct_readLowPowerPayloadIntervalWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configLowPowerPayloadInterval {
    __block BOOL success = NO;
    [MKCTInterface ct_configLowPowerPayloadInterval:[self.interval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
