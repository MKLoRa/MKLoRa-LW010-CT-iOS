...
@interface MKCTDeviceSettingController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCTDeviceSettingModel *dataModel;

@property (nonatomic, strong)NSArray *timeZoneList;

@end

@implementation MKCTDeviceSettingController

- (void)dealloc {
    NSLog(@"MKCTDeviceSettingController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 1 && indexPath.row == 0) {
        //Indicator Settings
        MKCTIndicatorSettingsController *vc = [[MKCTIndicatorSettingsController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 6 && indexPath.row == 0) {
        //ON/OFF Settings
        MKCTOnOffSettingsController *vc = [[MKCTOnOffSettingsController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 7 && indexPath.row == 0) {
        //Device Info
        MKCTDeviceInfoController *vc = [[MKCTDeviceInfoController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
...
- (void)mk_loraTextButtonCellSelected:(NSInteger)index
    ...
    if (index == 0) {
        ...
        self.dataModel.buzzer = dataListIndex;
        return;
    }
    if (index == 1) {
        //Current Time Zone
...
- (void)updateCellStates {
    MKTextButtonCellModel *buzzerModel = self.section0List[0];
...
- (void)factoryReset{
    ...
    [alertView showAlertWithTitle:@"Factory Reset" message:msg notificationName:@"mk_ct_needDismissAlert"];
}
...
- (void)sendResetCommandToDevice{
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                                     inView:self.view
                              isPenetration:NO];
    [MKCTInterface ct_factoryResetWithSucBlock:^{
        [[MKHudManager share] hide];
...
- (void)powerOff{
    ...
    [alertView addAction:cancelAction];
    [alertView addAction:confirmAction];
    [alertView showAlertWithTitle:@"Warning!" message:msg notificationName:@"mk_ct_needDismissAlert"];
}
...
- (void)commandPowerOff{
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                                     inView:self.view
                              isPenetration:NO];
    [MKCTInterface ct_powerOffWithSucBlock:^{
        [[MKHudManager share] hide];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-CT", @"MKCTDeviceSettingController", @"ct_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCTDeviceSettingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCTDeviceSettingModel alloc] init];
    }
    return _dataModel;
}
