...
@interface MKCTVibrationController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCTVibrationDataModel *dataModel;

@end

@implementation MKCTVibrationController

- (void)dealloc {
    NSLog(@"MKCTVibrationController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-CT", @"MKCTVibrationController", @"ct_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCTVibrationDataModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCTVibrationDataModel alloc] init];
    }
    return _dataModel;
}
