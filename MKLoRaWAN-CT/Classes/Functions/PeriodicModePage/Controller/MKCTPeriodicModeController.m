...
@interface MKCTPeriodicModeController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCTPeriodicModeModel *dataModel;

@end

@implementation MKCTPeriodicModeController

- (void)dealloc {
    NSLog(@"MKCTPeriodicModeController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-CT", @"MKCTPeriodicModeController", @"ct_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCTPeriodicModeModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCTPeriodicModeModel alloc] init];
    }
    return _dataModel;
}
