...
@interface MKCTTabBarController ()
...
@implementation MKCTTabBarController

- (void)dealloc {
    NSLog(@"MKCTTabBarController销毁");
...
- (void)viewDidDisappear:(BOOL)animated{
    ...
    if (![[self.navigationController viewControllers] containsObject:self]){
        [[MKCTCentralManager shared] disconnect];
    }
}
...
- (void)addNotifications {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(gotoScanPage)
                                                 name:@"mk_ct_popToRootViewControllerNotification"
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(dfuUpdateComplete)
                                                 name:@"mk_ct_centralDeallocNotification"
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(centralManagerStateChanged)
                                                 name:mk_ct_centralManagerStateChangedNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(disconnectTypeNotification:)
                                                 name:mk_ct_deviceDisconnectTypeNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(deviceEnterDebuggerMode)
                                                 name:@"mk_ct_startDebuggerMode"
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(deviceStopDebuggerMode)
                                                 name:@"mk_ct_stopDebuggerMode"
                                               object:nil];
}
...
- (void)gotoScanPage {
    ...
    [self dismissViewControllerAnimated:YES completion:^{
        @strongify(self);
        if ([self.delegate respondsToSelector:@selector(mk_ct_needResetScanDelegate:)]) {
            [self.delegate mk_ct_needResetScanDelegate:NO];
        }
    }];
}
...
- (void)dfuUpdateComplete {
    ...
    [self dismissViewControllerAnimated:YES completion:^{
        @strongify(self);
        if ([self.delegate respondsToSelector:@selector(mk_ct_needResetScanDelegate:)]) {
            [self.delegate mk_ct_needResetScanDelegate:YES];
        }
    }];
}
...
- (void)showAlertWithMsg:(NSString *)msg title:(NSString *)title{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_ct_needDismissAlert" object:nil];
    //让所有MKPickView消失
    ...
    [alertView showAlertWithTitle:title message:msg notificationName:@"mk_ct_needDismissAlert"];
}

- (void)loadSubPages {
    MKCTLoRaController *loraPage = [[MKCTLoRaController alloc] init];
    loraPage.tabBarItem.title = @"LORA";
    loraPage.tabBarItem.image = LOADICON(@"MKLoRaWAN-CT", @"MKCTTabBarController", @"ct_lora_tabBarUnselected.png");
    loraPage.tabBarItem.selectedImage = LOADICON(@"MKLoRaWAN-CT", @"MKCTTabBarController", @"ct_lora_tabBarSelected.png");
    MKBaseNavigationController *advNav = [[MKBaseNavigationController alloc] initWithRootViewController:loraPage];

    MKCTPositionController *positionPage = [[MKCTPositionController alloc] init];
    positionPage.tabBarItem.title = @"POSITION";
    positionPage.tabBarItem.image = LOADICON(@"MKLoRaWAN-CT", @"MKCTTabBarController", @"ct_position_tabBarUnselected.png");
    positionPage.tabBarItem.selectedImage = LOADICON(@"MKLoRaWAN-CT", @"MKCTTabBarController", @"ct_position_tabBarSelected.png");
    MKBaseNavigationController *positionNav = [[MKBaseNavigationController alloc] initWithRootViewController:positionPage];

    MKCTGeneralController *settingPage = [[MKCTGeneralController alloc] init];
    settingPage.tabBarItem.title = @"GENERAL";
    settingPage.tabBarItem.image = LOADICON(@"MKLoRaWAN-CT", @"MKCTTabBarController", @"ct_setting_tabBarUnselected.png");
    settingPage.tabBarItem.selectedImage = LOADICON(@"MKLoRaWAN-CT", @"MKCTTabBarController", @"ct_setting_tabBarSelected.png");
    MKBaseNavigationController *settingNav = [[MKBaseNavigationController alloc] initWithRootViewController:settingPage];
    
    MKCTDeviceSettingController *deviceInfo = [[MKCTDeviceSettingController alloc] init];
    deviceInfo.tabBarItem.title = @"DEVICE";
    deviceInfo.tabBarItem.image = LOADICON(@"MKLoRaWAN-CT", @"MKCTTabBarController", @"ct_device_tabBarUnselected.png");
    deviceInfo.tabBarItem.selectedImage = LOADICON(@"MKLoRaWAN-CT", @"MKCTTabBarController", @"ct_device_tabBarSelected.png");
