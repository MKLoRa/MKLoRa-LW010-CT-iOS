...
@interface MKCTAxisSettingController ()<UITableViewDelegate,
...
@implementation MKCTAxisSettingController

- (void)dealloc {
    NSLog(@"MKCTAxisSettingController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-CT", @"MKCTAxisSettingController", @"ct_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCTAxisSettingDataModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCTAxisSettingDataModel alloc] init];
    }
    return _dataModel;
}
