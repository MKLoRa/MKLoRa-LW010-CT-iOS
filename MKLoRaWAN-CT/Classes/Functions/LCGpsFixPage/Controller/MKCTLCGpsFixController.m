...
@implementation MKCTLCGpsFixController

- (void)dealloc {
    NSLog(@"MKCTLCGpsFixController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-CT", @"MKCTLCGpsFixController", @"ct_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCTLCGpsFixModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCTLCGpsFixModel alloc] init];
    }
    return _dataModel;
}
