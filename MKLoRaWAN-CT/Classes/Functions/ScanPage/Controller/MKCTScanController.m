...
static NSString *const localPasswordKey = @"mk_ct_passwordKey";

static CGFloat const searchButtonHeight = 40.f;

static NSTimeInterval const kRefreshInterval = 0.5f;

@interface MKCTScanController ()<UITableViewDelegate,
UITableViewDataSource,
mk_ct_centralManagerScanDelegate,
MKCTScanPageCellDelegate,
MKSearchButtonDelegate,
MKCTTabBarControllerDelegate>
...
@implementation MKCTScanController

- (void)dealloc {
    NSLog(@"MKCTScanController销毁");
    [[NSNotificationCenter defaultCenter] removeObserver:self];
    //移除runloop的监听
    CFRunLoopRemoveObserver(CFRunLoopGetCurrent(), self.observerRef, kCFRunLoopCommonModes);
    [[MKCTCentralManager shared] stopScan];
    [MKCTCentralManager removeFromCentralList];
}
...
- (void)viewDidLoad {
    ...
    [MKCTCentralManager shared].delegate = self;
...
- (void)rightButtonMethod {
    UIViewController *vc = [[CTMediator sharedInstance] CTMediator_LORAWAN_CT_AboutPage];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    MKCTScanPageCell *cell = [MKCTScanPageCell initCellWithTableView:self.tableView];
...
#pragma mark - mk_ct_centralManagerScanDelegate
- (void)mk_ct_receiveDevice:(NSDictionary *)deviceModel {
    MKCTScanPageModel *model = [MKCTScanPageModel mk_modelWithJSON:deviceModel];
...
- (void)refreshButtonPressed {
    if ([MKCTCentralManager shared].centralStatus != mk_ct_centralManagerStatusEnable) {
...
- (void)scanTimerRun{
    ...
    [[MKCTCentralManager shared] startScan];
...
- (void)updateDataWithScanModel:(MKCTScanPageModel *)scanDataModel{
 ...
 */
- (void)filterTrackerDataWithSearchName:(MKCTScanPageModel *)scanDataModel{
    if (scanDataModel.rssi < self.buttonModel.searchRssi) {
        return;
    }
...
- (void)processTrackerData:(MKCTScanPageModel *)scanDataModel{
    //查看数据源中是否已经存在相关设备
 ...
 */
- (void)dataNoExistDataSource:(MKCTScanPageModel *)scanDataModel{
 ...
 */
- (void)dataExistDataSource:(MKCTScanPageModel *)scanDataModel {
    NSInteger currentIndex = 0;
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        MKCTScanPageModel *dataModel = self.dataList[i];
        if ([dataModel.macAddress isEqualToString:scanDataModel.macAddress]) {
            currentIndex = i;
            break;
        }
    }
    MKCTScanPageModel *dataModel = self.dataList[currentIndex];
...
#pragma mark - 连接部分
- (void)connectDeviceWithModel:(MKCTScanPageModel *)scanDataModel {
    //停止扫描
    [self.refreshIcon.layer removeAnimationForKey:@"mk_refreshAnimationKey"];
    [[MKCTCentralManager shared] stopScan];
    if (self.scanTimer) {
    ...
    [alertView showAlertWithTitle:@"Enter password" message:msg notificationName:@"mk_ct_needDismissAlert"];
}

- (void)connectDeviceWithDataModel:(MKCTScanPageModel *)scanDataModel {
    ...
    [[MKCTConnectModel shared] connectDevice:scanDataModel.peripheral password:(scanDataModel.needPassword ? password : @"") deviceType:scanDataModel.deviceType sucBlock:^{
...
- (void)configParams {
    ...
    [[NSUserDefaults standardUserDefaults] setObject:@"" forKey:@"ct_readRecordDataDayNumKey"];
    //读取设备存储的扫描信息页面，显示的设备总共存储了多少条的扫描数据
    [[NSUserDefaults standardUserDefaults] setObject:@"" forKey:@"ct_recordDataTotalSumKey"];
    
    [MKCTDatabaseManager clearDataTable];
    [MKCTDatabaseManager initDataBase];
}

- (void)pushTabBarPage {
    MKCTTabBarController *vc = [[MKCTTabBarController alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-CT", @"MKCTScanController", @"ct_scanRightAboutIcon.png") forState:UIControlStateNormal];
...
- (UIImageView *)refreshIcon {
    if (!_refreshIcon) {
        ...
        _refreshIcon.image = LOADICON(@"MKLoRaWAN-CT", @"MKCTScanController", @"ct_scan_refreshIcon.png");
    }
    return _refreshIcon;
}
