...
@implementation MKCTScanPageCell

+ (MKCTScanPageCell *)initCellWithTableView:(UITableView *)tableView {
    MKCTScanPageCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCTScanPageCellIdenty"];
    if (!cell) {
        cell = [[MKCTScanPageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCTScanPageCellIdenty"];
    }
    return cell;
}
...
- (void)connectButtonPressed {
    if ([self.delegate respondsToSelector:@selector(ct_scanCellConnectButtonPressed:)]) {
        [self.delegate ct_scanCellConnectButtonPressed:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCTScanPageModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCTScanPageModel.class]) {
        return;
    }
    //顶部
    ...
    self.batteryLabel.text = [_dataModel.batteryPercent stringByAppendingString:@"%"];
...
- (UIImageView *)rssiIcon {
    if (!_rssiIcon) {
        ...
        _rssiIcon.image = LOADICON(@"MKLoRaWAN-CT", @"MKCTScanPageCell", @"ct_scan_rssiIcon.png");
    }
    return _rssiIcon;
}
...
- (UIImageView *)batteryIcon {
    if (!_batteryIcon) {
        ...
        _batteryIcon.image = LOADICON(@"MKLoRaWAN-CT", @"MKCTScanPageCell", @"ct_scan_batteryIcon.png");
    }
    return _batteryIcon;
}
