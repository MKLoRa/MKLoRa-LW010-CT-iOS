...
@interface MKCTOnOffSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCTOnOffSettingsModel *dataModel;

@end

@implementation MKCTOnOffSettingsController

- (void)dealloc {
    NSLog(@"MKCTOnOffSettingsController销毁");
}
...
- (void)configShutDownPayload:(BOOL)isOn {
    ...
    [MKCTInterface ct_configShutdownPayloadStatus:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (void)configOffByButton:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCTInterface ct_configHallPowerOffStatus:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (void)configAutoPowerOn:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCTInterface ct_configAutoPowerOnAfterCharging:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (MKCTOnOffSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCTOnOffSettingsModel alloc] init];
    }
    return _dataModel;
}
