...
@implementation MKCTBatteryInfoCellModel
@end

@interface MKCTBatteryInfoCell ()
...
@property (nonatomic, strong)UILabel *staticCountLabel;

@property (nonatomic, strong)UILabel *moveCountLabel;

@end

@implementation MKCTBatteryInfoCell

+ (MKCTBatteryInfoCell *)initCellWithTableView:(UITableView *)tableView {
    MKCTBatteryInfoCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCTBatteryInfoCellIdenty"];
    if (!cell) {
        cell = [[MKCTBatteryInfoCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCTBatteryInfoCellIdenty"];
    }
    return cell;
}
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        [self.contentView addSubview:self.staticCountLabel];
        [self.contentView addSubview:self.moveCountLabel];
    }
    return self;
}
...
- (void)layoutSubviews {
    ...
    [self.staticCountLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(15.f);
        make.right.mas_equalTo(-15.f);
        make.top.mas_equalTo(self.batteryPowerLabel.mas_bottom).mas_offset(10.f);
        make.height.mas_equalTo(MKFont(13.f).lineHeight);
    }];
    [self.moveCountLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(15.f);
        make.right.mas_equalTo(-15.f);
        make.top.mas_equalTo(self.staticCountLabel.mas_bottom).mas_offset(10.f);
        make.height.mas_equalTo(MKFont(13.f).lineHeight);
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCTBatteryInfoCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCTBatteryInfoCellModel.class]) {
        return;
    }
    ...
    self.staticCountLabel.text = [SafeStr(_dataModel.staticPositionCount) stringByAppendingString:@" peices of payload 1"];
    self.moveCountLabel.text = [SafeStr(_dataModel.movePositionCount) stringByAppendingString:@" peices of payload 2"];
}
...
- (UILabel *)staticCountLabel {
    if (!_staticCountLabel) {
        _staticCountLabel = [self fetchValueLabel];
    }
    return _staticCountLabel;
}

- (UILabel *)moveCountLabel {
    if (!_moveCountLabel) {
        _moveCountLabel = [self fetchValueLabel];
    }
    return _moveCountLabel;
}
